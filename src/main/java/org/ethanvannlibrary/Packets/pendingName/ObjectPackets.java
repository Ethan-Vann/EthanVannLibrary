...
public class ObjectPackets extends PacketReflectionDependent {

    public ObjectPackets(PacketReflection packetReflection) {
        super(packetReflection);
    }

    @SneakyThrows
    public void queueObjectAction(int actionFieldNo, int objectId, int worldPointX, int worldPointY,
                                  boolean ctrlDown) {
        int ctrl = ctrlDown ? 1 : 0;
        switch (actionFieldNo) {
            case 1:
                packetReflection.sendPacket(PacketDef.getOpLoc1(packetReflection), objectId, worldPointX, worldPointY, ctrl);
                break;
            case 2:
                packetReflection.sendPacket(PacketDef.getOpLoc2(packetReflection), objectId, worldPointX, worldPointY, ctrl);
                break;
            case 3:
                packetReflection.sendPacket(PacketDef.getOpLoc3(packetReflection), objectId, worldPointX, worldPointY, ctrl);
                break;
            case 4:
                packetReflection.sendPacket(PacketDef.getOpLoc4(packetReflection), objectId, worldPointX, worldPointY, ctrl);
                break;
            case 5:
                packetReflection.sendPacket(PacketDef.getOpLoc5(packetReflection), objectId, worldPointX, worldPointY, ctrl);
                break;
        }
    }
    ...
    public void queueWidgetOnTileObject(int objectId, int worldPointX, int worldPointY, int sourceSlot,
        ...
        int ctrl = ctrlDown ? 1 : 0;
        packetReflection.sendPacket(PacketDef.getOpLocT(packetReflection), objectId, worldPointX, worldPointY, sourceSlot, sourceItemId,
                sourceWidgetId, ctrl);
    }
